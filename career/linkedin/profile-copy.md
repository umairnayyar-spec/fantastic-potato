# LinkedIn Profile Copy — paste-ready

> For https://www.linkedin.com/in/umairkhan99/ — drafted 2026-07-14 from the
> master resume. Paste manually into LinkedIn (no automation).
> Status: DRAFT v2 — gap answers folded in (63→238 growth, <5% attrition,
> comp structures, education years, public trainings).

---

## Headline (pick one, max 220 chars)

**Variant A — broad senior leader (recommended):**
> Senior HR Leader | HRBP & Head of People | 14+ yrs · APAC, Europe & GCC | Ex-Epic Games | M&A Integration · Org Design · HR Ops & HRMIS (Workday, SAP)

**Variant B — HRBP-first (for HRBP/Director HRBP searches):**
> Senior HR Business Partner | Partnering VPs & C-suite across APAC, EU & Americas | Ex-Epic Games | Org Design · Talent · M&A Integration | Open to GCC, EU & Remote

**Variant C — builder angle (for startup/scale-up Head of People searches):**
> Head of People | Scaled Quixel 63→238 with <5% attrition | 3x senior-most HR leader | Ex-Epic Games | HR Ops · HRMIS · Org Design | 14+ yrs

---

## About (~200 words)

I've spent 14+ years doing the two halves of HR that rarely live in one
person: strategic partnering with VPs and C-suite — and building the HR
machinery underneath it, hands-on.

At Epic Games I was Senior HRBP for APAC, Europe and the Americas: 450+
employees, a team of 4 HRBPs, the HR integration of the Quixel acquisition,
restructurings across Product and Automotive, and benchmarking Epic's job
levels for the Pakistan territory with the comp team. Before that I was the
senior-most HR leader at Quixel as it scaled from 63 to 238 people — keeping
attrition under 5% the whole way, cutting hiring cycle time from 57 to 38
days, and moving 200+ people to a hybrid model that held productivity for
three years.

I've been the first senior People hire three times (Quixel, Trillium,
ImagineArt), and I've personally deployed HRMIS and ATS platforms —
Workday, SAP, Lever, Greenhouse — end to end.

What I'm looking for next: a senior People leadership mandate — HRBP
leadership, Head of People, or OD — in the GCC or Europe (relocation-ready),
remote, or Pakistan.

📧 umair.nayyar@gmail.com

---

## Experience section blurbs (2–4 lines each — LinkedIn is skimmed)

**Senior HR Partner — ImagineArt · Apr 2026 – Jun 2026**
Built the People & Culture function from zero for a high-growth AI product
company: HR operations, lifecycle workflows, HRMIS and self-service, and
SLA/KPI structures — a fixed build-out mandate, delivered and concluded.

**Independent HR Consultant · Jan 2026 – Mar 2026**
HRBP and org-design advisory for scaling companies across multiple time zones.

**Senior HR Business Partner (APAC, Europe & Americas) — Epic Games · Jan 2024 – Dec 2025**
Strategic partner to VPs and Directors across 450+ employees on three
continents. Ran performance management and employee relations across the
regions; led a team of 4 HRBPs, the post-acquisition HR integration of
Quixel (incl. Sweden), and restructurings of Product, Product Strategy and
Automotive. Co-led talent reviews and comp planning.

**Manager HR / Head of HR Pakistan — Quixel (Epic Games) · Sep 2019 – Dec 2023**
Senior-most HR leader as Quixel scaled 63→238 employees through acquisition
by Epic Games — with attrition held under 5%. Cut hiring cycle from 57→38
days (Lever ATS), built comp structures, implemented HRMIS + self-service
and MBO-based 360° performance, and moved 200+ employees to a hybrid model
that held productivity for three years.

**Senior Manager HR (Head of HR) — Trillium Information Security Systems · Apr 2016 – Aug 2019**
Owned people strategy and HR transformation: competency frameworks, HiPo and
succession programs, restructuring with 30% headcount optimization, 27-day
average hiring cycle.

**HR Business Partner, Commercial — Mobilink (VimpelCom) · Jul 2014 – Apr 2016**
HRBP to a 2,500-employee commercial division; cut Contact Centre North
turnover by 27%; established the HR analytics function.

**Learning, Development & OD Executive — Ufone (Etisalat) · Jul 2011 – Jun 2014**
Led the company-wide culture rollout ("The Ufone Way") across 2,500+
employees; ran HiPo programs and TNA.

---

## Skills (pin the top 3 ⭐)

⭐ HR Business Partnering · ⭐ Organizational Design · ⭐ HR Operations

Then: People Strategy, Workforce Planning, Talent Management, Succession
Planning, Performance Management, M&A Integration, Change Management,
Restructuring, Employee Relations, Retention Strategy, People Analytics,
HR Transformation, Shared Services, Employee Lifecycle, Onboarding,
Compensation Planning, HRIS/HRMIS, Workday, SAP, Lever, Greenhouse, 15Five,
Lattice, Organizational Development

## Languages section

English (Full professional) · Urdu (Native) · Punjabi (Limited working)

---

## Settings checklist (5 minutes, do once)

- [ ] **Open to Work** → recruiters-only mode. Titles: Senior HR Business
      Partner, HR Director, Head of People, Head of Organizational
      Development, People Operations Director. Locations: United Arab
      Emirates, Saudi Arabia, Qatar, Sweden, Pakistan + "Remote".
      Start date: "Immediately, I'm actively applying".
- [ ] Location field: keep "Islamabad, Pakistan" but add "Open to relocation
      (GCC · EU) & remote" to the headline (done in variants above).
- [ ] Custom URL: already good (umairkhan99).
- [ ] Photo: professional headshot; banner: anything non-default (even a
      clean solid color beats the grey default).
- [ ] Featured section: pin 1–2 items if available (talk, article, or a
      one-page "what I build" PDF).
- [ ] Ask 2–3 former colleagues (ideally Epic/Quixel leaders) for a short
      recommendation — recruiters read the top two.

## Education & certifications sections

- SZABIST — Master's in Human Resource Management (2013–2014)
- NUST Business School — BBA Hons. (2007–2011)
- Certifications to list: NLP Practitioner · Certified Behavioral
  Interviewer · Six Sigma Practitioner · Certified Training Facilitator ·
  Core Values Trainer · Situational Leadership · Radical Candor
- Consider one Featured/About line on public facilitation: "I also run
  public trainings on team building and people management."
